...
&= \int P(\theta|Y_{\text{obs}}, Y_{\text{mis}}) P(Y_{\text{mis}}|Y_{\text{obs}}) dY_{\text{mis}}  \\
&= \frac{1}{M} \lim_{{M \to \infty}} \sum_{m=1}^{M} P(\theta|Y_{\text{obs}}, Y^{(m)}_{\text{mis}})  \\
&\approx \frac{1}{M} \sum_{m=1}^{M} P(\theta|Y_{\text{obs}}, Y^{(m)}_{\text{mis}}) (\#eq:fm1)
\end{align*}
...
En las secciones anterior, se describió la imputación múltiple bajo el supuesto de ignorabilidad. Note que la sección (INTRODUCIR SECCION EN DONDE SE TOCA ESTE TEMA) trata el tema de los datos faltantes a partir de la distribución de $P(Y_\text{mis}|Y_\text{obs},R)$, lo que quiere decir que la distribución condicional de $Y_\text{mis}$ depende de los datos observados $Y_\text{obs}$ y del mecanismo de respuesta $R$. En otras palabras, los datos faltantes no solo dependen de los datos observados, si no que también depende de cúales valores son faltantes y cuáles fueron observados. En las siguientes secciones, se utilizarán paquetes estadísticos para ejemplificar procesos de imputación múltiple, dichos paquetes asumen la "ignorabilidad" en el sentido que omiten la matriz $R$ en los cálculos de las distribuciones [@van2018flexible]. Asumir lo anterior, para la mayoría de los casos prácticos es equivalente al concepto MAR descrito en este documento. 

# Métodos de imputación

## Algoritmo de Imputación múltiple bayesiana

Asumiendo que la ausencia de datos es ignorables, la ecuacion \@ref(eq:fm1) establece que las múltiples imputaciones para valores faltantes deben extraerse de su distribución predictiva posteriori como

\begin{equation}
Y^{(m)}_{\text{mis}} \sim P(Y_{\text{mis}} | Y_{\text{obs}}), \ \ m = 1, \ldots, M, (\#eq:fmbayes1)
\end{equation}

La ecuación \@ref(eq:fmbayes1) muestra que se debiese imputar valores faltantes como extracciones aleatorias de ciertas distribuciones derivadas bajo el modelo de imputación. Por lo tanto, la idea de múltiples imputaciones no es estimar o predecir valores faltantes a partir de ecuaciones, [@little_rubin_2020, Capítulo 4] proporcionaron más detalles sobre las distinciones entre dos estrategias de imputación (es decir, utilizando extracciones vs. utilizando predicciones).

Desde la perspectiva bayesiana, se necesita además una distribución a priori para $\theta$, $\pi(\theta)$, para completar la especificación del modelo. Formular directamente $P(Y_{\text{mis}} | Y_{\text{obs}})$ es difícil pues $\theta$ no está incluido en la distribución predictiva posteriori. Sin embargo, el algoritmo de imputación tiene que involucrar $\theta$ de alguna manera. 

Considere la siguiente ecuación,

\begin{equation}
P(Y_{\text{mis}} | Y_{\text{obs}}) = \int P(Y_{\text{mis}}, \theta | Y_{\text{obs}}) d\theta = \int P(Y_{\text{mis}} | Y_{\text{obs}}, \theta) P(\theta | Y_{\text{obs}}) d\theta. (\#eq:fmbayes2)
\end{equation}

La ecuación \@ref(eq:fmbayes1) sugiere que, condicional a una extracción de $\theta$, $Y_{\text{mis}}$ se puede extraer de $P(Y_{\text{mis}} | Y_{\text{obs}}, \theta)$. @he2021multiple muestran la estrategia como un algoritmo llamado direct Bayesian (DB) imputation algorithm que se muestra a continuación: 

\begin{enumerate}
\item Derivar la distribución posterior de los datos observados, $P(\theta | Y_{\text{obs}})$, bajo un prior $\pi(\theta)$.

\item Extraer un valor del parámetro $\theta^{(m)}$ de $P(\theta | Y_{\text{obs}})$.

\item Extraer valores de datos faltantes de la distribución condicional: $Y^{(m)}_{\text{mis}} \sim P(Y_{\text{mis}} | Y_{\text{obs}}, \theta^{(m)})$.

\item Repetir los Pasos 2 y 3 de forma independiente $M $ veces para crear múltiples conjuntos de datos completos $(Y^{(m)}_{\text{mis}}, Y_{\text{obs}})$ ($m = 1, \ldots, M$).

\end{enumerate}

En muchos casos, sin embargo, derivar $P(\theta | Y_{\text{obs}})$ y extraer parámetros de ella puede ser difícil si el patrón de datos faltantes no es simple o si el modelo de datos completos es complicado. Un algoritmo de imputación bayesiana más general es la estrategia llamada data augmentation (DA).
