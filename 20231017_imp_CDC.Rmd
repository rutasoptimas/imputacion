---
title: "Guía con lineamientos y orientaciones metodológicas para la imputación de datos estadísticos"
author:  |
  |
  |
  |
...
## Evaluación de usos y constumbres vigentes en la institución
...
footnote(general = "Esta tabla busca ilustrar la realidad de uso de estos métodos y señalar la frecuencia con que son invocados, marcando la necesidad de documentación oficial, por una parte y por otra, consolidar dichos métodos a nivel institucional.", threeparttable = T,general_title = "Nota: ")

```

## Mapa conceptual 

Conociendo nuestra realidad quedamos en posición de poder orientarnos usando como directrices los alcances y logros presentes en el estado del arte de esta importante técnica, que nos permitímos exhibir mediante el siguiente diagrama conceptual, para dar el contexto necesario acerca de que hacemos y la preponderancia de los métodos usados en la institición. 
...
\begin{equation}
y = \alpha + \beta_1 x_1 + \ldots + \beta_p x_p + \varepsilon^* = \alpha + \mathbf{x}^T \boldsymbol{\beta} + \varepsilon^*
 (\#eq:729)
\end{equation}
...
**III**. La varianza es proporcional a una variable auxiliar dentro de los grupos. En este caso $\nu_i = \left(\mathbf{z}_i \cdot \mathbf{x}_{ij}\right)^T \mathbf{c}$, siendo $\mathbf{c}$ un vector con constantes de proporcionalidad.
...
En el lenguaje de programació R, utilizamos herramientas como `tidyverse` y el paquete R de `naniar` para enseñar a manejar y analizar datos faltantes de manera efectiva. El paquete `naniar` es una herramienta muy útil para explorar, visualizar y manejar valores faltantes en R.
...
dat_hw <- read.table("C:/Users/jbustos/Documents/2023/Respaldo/Servidor_1/Respaldo14092023/edi_imp/datacamp_JB/dealing/dat_hw.txt", h=T,  dec=".")
```
...
library(dplyr)
library(ggplot2)
biopics <- read.csv("C:/Users/jbustos/Documents/2023/Respaldo/Servidor_1/Respaldo14092023/edi_imp/docu/curso_imputacion/biopics.csv")
```
...
# transformamos character a factor
...
```{r}
africa <- read.csv("C:/Users/jbustos/Documents/2023/Respaldo/Servidor_1/Respaldo14092023/edi_imp/datacamp_JB/handing/africa.csv", sep = ";")
```
